...
$(function() {
    ...
    function findData(dataType){
        ...
        $('#pinFirst').append(dataType[5].Name);
    ...
    function findData(dataType){
        var code = parseInt($('#queueCode').val());
            for(var i = 0 ; i<dataType.length ; i++){
                console.log(dataType[i].Code +" "+code);
                if(dataType[i].Code == code){
                   
                        var ID = parseInt(dataQueueId[5]);
                        console.log(ID +" "+ dataType[i].Id);
                        if(ID == dataType[i].Id){
                            console.log(ID+"  "+dataType[i].Id);
...
function send(){
    $.ajax({
        ...
        window.location.href = 'FoodMenu.html?id='+dataType[5].IdQueue;
    });
}